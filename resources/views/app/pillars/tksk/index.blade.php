    ...
    <div class="section-body">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a class="btn btn-primary ml-2" href="{{ route('app.pillar.tksk.import') }}" type="button">
                        ...
                        <tbody>
                            @foreach($tksks as $key => $tksk)
                                <tr>
                                    ...
                                    <td>{{ $tksk->duty_address ? $tksk->duty_address['regency'] : '' }} , {{ $tksk->duty_address ? $tksk->duty_address['district'] : '' }}</td>
                                    <td>{{ $tksk->name }}</td>
                                    ...
                                    <td @if(auth()->user()->hasRole('super-admin')) @endif>
                                        <div class="d-flex flex-row flex-wrap">
                                            <a href="{{ route('app.pillar.tksk.show', $tksk->hash) }}" class="btn btn-sm btn-info w-100" title="Detail">Detail Data</a>
                                            <a href="{{ route('app.pillar.tksk.personal.report', $tksk->hash) }}" class="btn btn-sm btn-warning w-100 mt-2" title="Detail Laporan">Detail Laporan</a>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-row flex-wrap">
                                            <a href="{{ route('app.pillar.tksk.edit', $tksk->hash) }}" class="btn btn-sm btn-primary w-100" title="Edit">Edit Data</a>
                                            <div class="w-100">
                                                <form action="{{ route('app.pillar.tksk.delete', $tksk->hash) }}" class="formDelete" method="POST">
                                                    @csrf
                                                    @method('delete')
