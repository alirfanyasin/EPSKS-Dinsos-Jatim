    ...
    <div class="section-body">
      ...
      <div class="card">
        <div class="card-header">
          ...
          {{-- <a class="ml-2 btn btn-primary" href="#" type="button">
              ...
              <tbody>
                @foreach ($psms as $key => $psm)
                  <tr>
                    ...
                    <td>{{ $psm->duty_address ? $psm->duty_address['regency'] : '' }} ,
                      {{ $psm->duty_address ? $psm->duty_address['district'] : '' }}</td>
                    <td @if (auth()->user()->hasRole('super-admin'))  @endif>
                      <div class="flex-row flex-wrap d-flex">
                        <a href="{{ route('app.pillar.psm.profile.show', $psm->hash) }}" class="btn btn-sm btn-info w-100"
                          title="Detail">Detail Data</a>
                        <a href="{{ route('app.pillar.psm.profile.report', $psm->hash) }}"
                          class="mt-2 btn btn-sm btn-warning w-100" title="Detail Laporan">Detail Laporan</a>
                      </div>
                    </td>
                    <td>
                      <div class="flex-row flex-wrap d-flex">
                        <a href="{{ route('app.pillar.psm.profile.edit', $psm->hash) }}"
                          class="btn btn-sm btn-primary w-100" title="Edit">Edit Data</a>
                        <div class="w-100">
                          <form action="{{ route('app.pillar.psm.profile.delete', $psm->hash) }}" class="formDelete"
                            method="POST">
                            @csrf
                            @method('delete')
